...
export class BIMPortalClient {
  ...
  constructor(opts: BIMPortalClientOptions = {}) {
    this.baseUrl = (opts.baseUrl ?? process.env.BIM_PORTAL_BASE ?? 'https://via.bund.de/bim').replace(/\/$/, '');
    this.accessToken = opts.accessToken ?? process.env.BIM_PORTAL_TOKEN ?? undefined;
    this.userAgent = opts.userAgent ?? 'ifc-normalizer/1.0';
  }
  ...
  getPropertyByGuid(guid: string) {
    return this.request<any>(`/merkmale/api/v1/public/property/${encodeURIComponent(guid)}`);
  }

  /** Search public property groups */
  searchPropertyGroups(params: Query) {
    return this.request<any>('/merkmale/api/v1/public/propertygroup', {}, {
      method: 'POST',
      body: JSON.stringify(params)
    });
  }
  ...
  getPropertyGroupByGuid(guid: string) {
    return this.request<any>(`/merkmale/api/v1/public/propertygroup/${encodeURIComponent(guid)}`);
  }
}
